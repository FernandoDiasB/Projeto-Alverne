function Carousel(config) {
    ...
    function init() {
        if (_this.itens.length === 0) {
            console.error('No items found for the carousel');
            return;
        }
    ...
    function showNextSlide() {
        _currentSlide = (_currentSlide + 1) % _this.itens.length;
        showSlide();
    }

    function showPrevSlide() {
        _currentSlide = (_currentSlide - 1 + _this.itens.length) % _this.itens.length;
        showSlide();
    }
